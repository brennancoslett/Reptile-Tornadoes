...
def calcFrameEnergies(file_path: Path, weightForHFC=False):
    ...
    frame_energies_norm = frame_energies - frame_energies.real.min() - 1j * \
        frame_energies.imag.min()
    frame_energies_norm = (frame_energies_norm /
                           np.abs(frame_energies).max()).real
...
def evalFunc(predictFilePathList, gtFilePathList, evalType):
    ...
    if evalType == "tempo":
        pass
        print(
            "needs to be ported from JavaFramwork. EvalTempo may need to be a new function")
    else:
        for i, file in enumerate(predictFilePathList):
            ...
            while(truthCursor < len(gtValues) and predictCursor < len(prValues)):
                ...
                if error < tolerance:
                    ...
                    raise RuntimeError(
                        f"Cannot match gt {trueEvent} with prediction {predictEvent}")
