...
public class Runner {
	...
	public static void main(String[] args) throws Exception {
		...
		if (options.has("p")) {
			...
			Files.walk(Paths.get(directory))
				...
				.filter(path -> path.toString()
						.endsWith(AUDIO)).forEach(path -> predictForFile(path.toString()));
		}
		...
		if (options.has("e")) {
			...
			Files.walk(Paths.get(directory))
				.filter(path -> Files.isRegularFile(path))
				.filter(path -> path.toString()
						.endsWith(AUDIO)).forEach(path -> evalForFile(path.toString()));
		}
		...
		if (options.has("s")) {
			...
			for (String type : types) {
				if(type == TEMPO)
				{
					...
					.filter(path -> Files.isRegularFile(path))
					.filter(path -> path.toString()
							.endsWith(type + EV)).forEach(path -> updateSummaryForFile(summary, path.toString()));

					String summaryfilename = directory + "/summary" + type + EV + ".txt";
					Utils.dictToFile(summaryfilename, summary);
				} else {
					Map<String, Integer> summary = new HashMap<>();
					summary.put("tp", 0);
					summary.put("fp", 0);
	...
	private static Processor findAndInstantiateClass(String processorName) {
		...
		try {
			...
			System.out.println(String.format("Instantiating class '%s' ...", fullyQualifiedName));
	...
	private static void updateSummaryForFile(Map<String, Integer> summary, String filename) {
		...
		for(Map.Entry<String, Integer> entry: summary.entrySet()) {
			summary.put(entry.getKey(), summary.get(entry.getKey()) + dict.getOrDefault(entry.getKey(), 0));
		}
	}
	...
	private static void predictForFile(String filename) {
		...
		try {
			...
			System.out.println("Prediction failed due to some Exception (ignoring)...");
		}
	}
	...
	private static void evalForFile(String filename) {
		...
		for (int i = 0; i < types.length; i++) {
			...
			if (Files.exists(Paths.get(gt))) {
				String msg = "";
				if(!Files.exists(Paths.get(pr)))
					msg = " No predictions found !";
				String ev = prefix + type + EV;
				System.out.println("Evaluating '" + pr + "' ..." + msg);
	 ...
	 */
	public static void evaluateEventFiles(String type, String groundtruth, String predictions, String evaluation, double tolerance) {
		...
		if (type.equals(Runner.TEMPO)) {
			...
			List<Double> eventGT = Utils.filterNaNandInf(rawGT);

			List<Double> rawPR = Utils.listFromFile(predictions);
			List<Double> eventPR = Utils.filterNaNandInf(rawPR);
			Map<String, Integer> summary = Utils.evaluateTempo(eventGT, eventPR, tolerance);

			Utils.dictToFile(evaluation, summary);
		} else {
			List<Double> rawGT = Utils.listFromFile(groundtruth);
			List<Double> eventGT = Utils.cleanEventList(Utils.filterNaNandInf(rawGT), tolerance);

			List<Double> rawPR = Utils.listFromFile(predictions);
			List<Double> eventPR = Utils.cleanEventList(Utils.filterNaNandInf(rawPR), tolerance);
